...
## ⛔ KHÔNG CHEN NGANG — TUYỆT ĐỐI

**Sau khi spawn sub-agent, Commander PHẢI dùng `sessions_yield` để chờ.**
...
- ❌ SAI (CHEN NGANG — tốn token, đứt mạch tư duy):
  1. Spawn sub-agent
  2. Trong lúc chờ, tự làm luôn công việc của sub-agent
  3. Sub-agent hoàn thành → code bị ghi đè, conflict
```

**Tại sao:**
- Sub-agent dùng Pro model → output chất lượng cao hơn Commander
- Chen ngang = lãng phí token (cả Commander + Sub-agent cùng làm 1 việc)
- Gây conflict file (Commander và Sub-agent ghi đè lên nhau)
- Đứt mạch tư duy của Sub-agent (đang suy nghĩ bị gián đoạn)

**Nếu sub-agent quá chậm (>5 phút):** Tăng `runTimeoutSeconds` lên 600, không tự làm thay.
